...
_file_io_executor = ThreadPoolExecutor(
    max_workers=_max_workers, thread_name_prefix="file_io"
)
...
async def upload_files(
    ...
    async def save_file_async(file: UploadFile, index: int):
        ...
        relative_path = file.filename or ""
        actual_filename = (
            os.path.basename(relative_path) if relative_path else file.filename
        )
        dir_path = os.path.dirname(relative_path) if relative_path else ""
        ...
        if (
            original_created_at
            and index < len(original_created_at)
            and original_created_at[index]
        ):
        ...
        if (
            original_updated_at
            and index < len(original_updated_at)
            and original_updated_at[index]
        ):
    ...
    if file_data_list:
        ...
        stmt = (
            select(File)
            .where(
                File.storage_path.in_(storage_paths), File.user_id == current_user.id
            )
...
async def move_file(
    ...
    if file_move.folder_id:
        folder_stmt = select(Folder).where(
            Folder.id == file_move.folder_id, Folder.user_id == current_user.id
        )
...
async def list_files(
    ...
    file_type: Optional[str] = Query(
        None, description="文件类型过滤: image, video, audio, document, pdf"
    ),
    ...
    if file_type:
        if file_type == "image":
            ...
            stmt = stmt.where(
                or_(File.mime_type.like("%document%"), File.mime_type.like("%word%"))
            )

    query = stmt.offset(offset).limit(page_size).order_by(File.created_at.desc())
...
async def batch_move_files(
    ...
    if batch_move.folder_id:
        folder_stmt = select(Folder).where(
            Folder.id == batch_move.folder_id, Folder.user_id == current_user.id
        )
        folder_res = await db.execute(folder_stmt)
        if not folder_res.scalar_one_or_none():
            raise HTTPException(status_code=400, detail="Target folder not found")
    ...
    stmt = select(File).where(
        File.id.in_(batch_move.file_ids), File.user_id == current_user.id
    )
    result = await db.execute(stmt)
    files = result.scalars().all()
...
async def batch_delete_files(
    ...
    stmt = select(File).where(
        File.id.in_(batch_op.file_ids), File.user_id == current_user.id
    )
    result = await db.execute(stmt)
    files = result.scalars().all()
