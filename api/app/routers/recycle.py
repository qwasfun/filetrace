...
from app.services.storage import delete_file as delete_file_storage, get_public_url
...
async def list_recycle_bin_items(
    ...
    # Get deleted folders
    folder_stmt = select(Folder).where(Folder.user_id == current_user.id, Folder.is_deleted == 1)
    folder_res = await db.execute(folder_stmt)
    folders = folder_res.scalars().all()

    # Get deleted files
    file_stmt = select(File).where(File.user_id == current_user.id, File.is_deleted == 1)
...
async def restore_items(
    ...
    if request.file_ids:
        stmt = select(File).where(File.id.in_(request.file_ids), File.user_id == current_user.id)
