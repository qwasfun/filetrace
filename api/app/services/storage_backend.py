...
class S3StorageBackend(StorageBackend):
            ...
            if error_code == '404':
                try:
                    ...
                    if self.endpoint_url:
                        ...
                        if hasattr(self, 'region_name') and self.region_name != 'us-east-1':
                    ...
                    if 'SecondLevelDomainForbidden' in error_msg or 'BucketAlreadyExists' in error_msg or 'BucketAlreadyOwnedByYou' in error_msg:
                        print(f"S3 桶 '{self.bucket_name}' 可能已存在或需要手动创建: {error_msg}")
                    else:
    ...
    def save(self, file: UploadFile, user_id: str = None) -> Tuple[str, int, dict]:
        ...
        # 检测文件类型（使用完整内容或前8KB）
        detection_content = file_content[:8192] if len(file_content) > 8192 else file_content
        file_type_info = FileTypeDetector.detect(
            filename=file.filename,
